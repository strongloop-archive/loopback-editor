var path = require('path'),
    moduleLoader = require('sl-module-loader').create(path.join(__dirname, '../editor-core')),
    Project = require('loopback-project-manager').Project,
...
module.exports = function initEditor(app) {
  ...
  function getProject(req, res, next) {
    ...
    var generators = moduleLoader.instanceOf('LoopbackEditorGenerator').map(function(g) {
      return {
    ...
    var editorUtils = moduleLoader.instanceOf('LoopbackEditorUtility').map(function(u) {
      return u;
    });
  ...
  moduleLoader.load(function(err, modules) {
    ...
    initBasicExtensions(app, moduleLoader, 'LoopbackEditorGenerator', 'generator');
    initBasicExtensions(app, moduleLoader, 'LoopbackEditorUtility', 'utility', function(req, res, next) {
