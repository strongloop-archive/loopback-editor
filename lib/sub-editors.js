...
module.exports = function initSubEditors(app, moduleLoader) {
  ...
  moduleLoader.instanceOf('LoopbackSubEditor').forEach(function(subEditor) {
    ...
    req.project.getConfig(function(err, config) {
      ...
      if (obj) {
        ...
        if (subEditor && subEditor.options.app) {
          subEditor.handle(req.params[1] ? '/' + req.params[1] : '/',
            '/project/' + req.params.project + '/object/' + req.params[0] + '/edit', req, res, next);
        } else {
          // Default editor
